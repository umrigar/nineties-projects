...
static Count buildNextHdr PROTO((Char *text, Size maxWidth, Boolean doForce,
  ...
  DocState *docStateP;
{
  ...
  if (docStateP->doc != NULL) {
	...
	case HI_BOUND_HELP: {
	  ...
	  assert(ranges[0] == 0);
	  assert(docStateP->checkPIndex == 1 /* lo-bound */ || \
		 docStateP->checkPIndex == 2 /* hi-bound */);
	  if (dest + totalLen < limit) {
	    sprintf(dest, "`%ld'", val);
